...
- Atlas has two main development services: the ASP.NET Core API in `src/backend/Api/Atlas.Api` and the Blazor WebAssembly UI in `src/frontend/Atlas.Ui`. Standard UI commands are documented in `src/frontend/Atlas.Ui/README.md`; Playwright e2e lives in `tests/e2e/` (`npm run test:e2e`); backend ports are documented in `src/backend/Api/Atlas.Api/Properties/launchSettings.json`.
