...
static class GrowthUiHelpers
{
    ...
    public static bool IsValidCreatedId(Guid? id) => id is { } value && value != Guid.Empty;
    ...
    public static bool IsTargetBeforeStart(string? startIso, string? targetIso)
    {
        ...
        if (!DateTimeOffset.TryParse(startIso, out DateTimeOffset start))
        {
            return false;
        }

        if (!DateTimeOffset.TryParse(targetIso, out DateTimeOffset target))
        {
            return false;
        }
